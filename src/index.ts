...
class TimelineCompiler<Description extends Cloneable, User extends Cloneable, FormalReason extends Cloneable> {
  ...
  private instants: Array<Instant<Description, User, FormalReason>> = []
  private currentInstant: Instant<Description, User, FormalReason> = NewInstant('')

  /**
     * Reduce applies event to the current state.
     * Events *must* be applied in order of compareEvent.
    */
  private reduce (event: ModelEvent<Description, User, FormalReason>): void {
  ...
  /**
     ...
     * @param nextDate
     */
  private finalizeInstant (nextDate?: DateDate): void {
